...
**Every written document MUST be reviewed by the proofreading agent before any commit or PR.**
...
- **Overflow issues** -- overfull hbox in LaTeX, content exceeding slide boundaries in Quarto
- **Consistency** -- notation, citation style (`\citet` vs `\citep` in LaTeX, `@key` vs `[@key]` in Quarto), terminology
...
Launch parallel review agents for all modified written documents. Each agent:
...
3. Produces a **detailed report** listing every proposed change with:
   - Line number or section/slide context
   - Current text (what's wrong)
   - Proposed fix (what it should be)
   - Category (grammar / typo / overflow / consistency)
4. Saves the report to `quality_reports/` (e.g., `quality_reports/[filename]_proofread_report.md`)
5. **Does NOT modify any source files**

**Example report entry:**
```
### Issue 7: Wrong preposition
- **File:** manuscript/3_results.tex
- **Location:** Section "Main Results", line ~247
...
```
You are an expert proofreading agent for academic writing. Review the file
...
4. CONSISTENCY: Citation format, notation, terminology
...
1. **Before any commit** -- on all modified written documents
2. **Before any PR** -- full sweep of all written documents
3. **After creating new content** -- immediately after drafting
...
```
quality_reports/
├── main_proofread_report.md
├── lecture01_topic_proofread_report.md
├── 3_results_proofread_report.md
└── ...
```
