...
4. **Store gold standard** in one of:
   - `quality_reports/[paper_author]_replication_targets.md` (for reference)
...
4. **Save all intermediate results as RDS:**
   - Raw data after cleaning: `temp_data/[paper]_data_clean.rds`
   - Estimation results: `temp_data/[paper]_estimates.rds`
   - Summary tables: `temp_data/[paper]_summary.rds`
...
Save to `quality_reports/[paper_author]_replication_report.md`:

```markdown
# Replication Report: [Paper Author (Year)]
**Date:** [YYYY-MM-DD]
**Context:** [paper / lecture / project]
...
2. **Now extend** with project-specific modifications:
   ...
   - Create project-specific figures
...
- **domain-reviewer:** Checks code-theory alignment
